...
export default function Concurso() {
    ...
    const fetchDrawings = async () => {
        const data = await getDrawings('CONCURSO');
    ...
    return (
        <div className="space-y-12">
            <header className="text-center md:text-left flex flex-col md:flex-row md:items-end justify-between gap-6 pb-8 border-b border-rose-100 dark:border-rose-900/30">
                <div className="space-y-4">
                    <h1 className="text-5xl md:text-6xl font-friendly text-[#1e1219] dark:text-white tracking-tight">
                        Galería de <span className="text-[#bf152d] dark:text-[#ff4d6d]">Arte</span>
                    </h1>
                    <p className="text-[#41495b] dark:text-slate-400 max-w-lg font-light text-lg">
                        Creatividad navideña en su máxima expresión. <br />
                        <span className="text-[#c6416a] dark:text-[#ff8fa3] font-medium">¡Vota por tus favoritos!</span>
                    </p>
                </div>
                ...
                <div className="flex items-center gap-3">
                    <button
                        ...
                        className="p-3 rounded-2xl bg-white dark:bg-zinc-900 border border-slate-100 dark:border-zinc-800 text-slate-400 hover:text-blue-500 dark:hover:text-blue-400 hover:shadow-lg transition-all"
            ...
            <InfoModal
                ...
                title="Reglas del Concurso de Arte"
            >
                <ul className="space-y-3">
                    <li className="flex gap-2">
                        <span className="text-[#bf152d] font-bold">•</span>
                        <span>Solo se permite material original relacionado con la navidad.</span>
                    </li>
                    <li className="flex gap-2">
                        <span className="text-[#bf152d] font-bold">•</span>
                        <span>Puedes participar con un solo dibujo o diseño.</span>
                    </li>
                    <li className="flex gap-2">
                        <span className="text-[#bf152d] font-bold">•</span>
                        <span>¡Los ganadores se mostrarán en tiempo real en el Ranking!</span>
                    </li>
                    <li className="flex gap-2">
                        <span className="text-[#bf152d] font-bold">•</span>
                        <span>Fomentamos el respeto; cualquier contenido inapropiado será eliminado.</span>
                    </li>
                </ul>
            </InfoModal>
            ...
            {showUpload && (
                <div className="bg-white dark:bg-slate-800 rounded-3xl p-8 relative animate-in fade-in slide-in-from-bottom-4 duration-500 shadow-2xl shadow-red-100 dark:shadow-red-900/10 border border-rose-100 dark:border-slate-700">
                    ...
                    <UploadDrawing category="CONCURSO" onUploadSuccess={() => { fetchDrawings(); setShowUpload(false); }} />
                </div>
            )}
            ...
            {loading ? (
                ...
                <div className="text-center py-32 border-2 border-dashed border-rose-200 dark:border-slate-800 rounded-3xl bg-white dark:bg-slate-900/50">
                    <p className="text-[#41495b] dark:text-slate-400 font-light text-xl">La galería está vacía.</p>
                    <p className="text-[#c6416a] dark:text-[#ff8fa3] mt-2">¡Sé el primero en inspirarnos!</p>
                </div>
            ) : (
